...
const addColorGradient = (color1, color2) => {
  ...
  div.style.background = `linear-gradient(90deg, ${toRGB(color1)} 0%, ${toRGB(
    color2,
  )} 100%)`;
...
for (let i = 0; i < gradientScale.length; i++) {
  backgroundColor += `${toRGB(gradientColor[i])} ${gradientScale[i]}%${
    i !== gradientScale.length - 1 ? ',' : ''
  } `;
}
backgroundColor += ')';
...
const updateFrame = (value) => {
  ...
  for (let i = 0; i < colors.length; i++) {
    ...
    document
      .getElementById('colors-code')
      .append(element, i < colors.length - 1 ? ',' : '');
  }
...
document.getElementById('gradients').addEventListener('mousemove', (event) => {
  ...
  const position =
    x - document.getElementById('gradients').getBoundingClientRect().x;
