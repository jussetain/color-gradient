...
export default class Gradient {
  ...
  getSubGradient = (value) => {
    ...
    const subgradientIndex = Math.min(
      Math.trunc((value * (this.colors.length - 1)) / 100),
   ...
   */
  getColorsScale = () =>
    this.colors.map((_, index) => (index * 100) / (this.colors.length - 1));
  ...
  getColorFromValue = (value, format = 'hex') => {
    ...
    const totalWeight = Math.round(
      subGradient.to.value - subGradient.from.value,
    );

    const leftWeight = Math.round(
      totalWeight - Math.abs(subGradient.from.value - value),
    );
    const rightWeight = Math.round(totalWeight - leftWeight);

    const red = Math.round(
      this.getWeightedAverage(
        subGradient.from.color.r,
        leftWeight,
        subGradient.to.color.r,
        rightWeight,
      ),
    );
    const green = Math.round(
      this.getWeightedAverage(
        subGradient.from.color.g,
        leftWeight,
        subGradient.to.color.g,
        rightWeight,
      ),
    );
    const blue = Math.round(
      this.getWeightedAverage(
        subGradient.from.color.b,
        leftWeight,
        subGradient.to.color.b,
        rightWeight,
      ),
    );

    if (format === 'hex') {
      return `#${Color.decToHex(red)}${Color.decToHex(green)}${Color.decToHex(
        blue,
      )}`;
    } else if (format === 'rgb') {
